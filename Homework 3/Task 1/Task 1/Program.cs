...
Console.WriteLine("People's first name starting with 'R', ordered by age (DESCENDING):");
List<Person> namesWithRSortedByAge = people.Where(n => n.FirstName[0] == 'R').OrderBy(a => a.Age).ToList();
PrintPerson(namesWithRSortedByAge);

Console.WriteLine("\nBrown dogs older than 3, sorted by age (ASCENDING):");
List<Dog> brownDogsOlderThanThree = dogs.Where(d => d.Age > 3 && d.Color == "Brown").OrderBy(a => a.Age).ToList();
PrintDogs(brownDogsOlderThanThree);

Console.WriteLine("\nPeople that own more than 2 dogs, sorted by name (DESCENDING):");
List<Person> ownMoreThanTwoDogs = people.Where(p => p.Dogs.Count > 2).OrderByDescending(n => n.FirstName).ToList();
PrintPerson(ownMoreThanTwoDogs);

Console.WriteLine("\nFreddy's dogs older than 1 year:");
List<Dog> freddysDogsOverOne = people.Where(n => n.FirstName == "Freddy").SelectMany(d => d.Dogs).Where(a => a.Age > 1).ToList();
PrintDogs(freddysDogsOverOne);

Dog nathensFirstDog = people.Where(n => n.FirstName == "Nathen").SelectMany(d => d.Dogs).ToList().First();
Console.WriteLine($"\nNathen's First dog is: {nathensFirstDog.PrintDogInfo()}");
// PrintDogs(nathensFirstDog);

Console.WriteLine("\nAll white dogs from Cristofer, Freddy, Erin and Amelia, ordered by name (ASCENDING):");
List<Dog> whiteDogs = people.Where(n => n.FirstName == "Cristofer" || n.FirstName == "Freddy"
									|| n.FirstName == "Erin" || n.FirstName == "Amelia")
							.SelectMany(d => d.Dogs).Where(c => c.Color == "White").OrderBy(n => n.Name).ToList();
PrintDogs(whiteDogs);
...
void PrintDogs(List<Dog> dogs)
{
    ...
    {
		Console.WriteLine(dog.PrintDogInfo());
	}
}
